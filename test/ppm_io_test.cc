...
namespace ppm_io = thinks::ppm_io;
...
TEST_CASE("Write invalid filename throws")
{
  ...
  REQUIRE_THROWS_AS(
    ppm_io::WriteRgbImage(filename, width, height, pixel_data),
    std::runtime_error);
}


TEST_CASE("Write invalid width throws")
{
  ...
  REQUIRE_THROWS_MATCHES(
    ppm_io::WriteRgbImage(oss, width, height, pixel_data),
    std::invalid_argument,
    utils::ExceptionContentMatcher("width must be non-zero"));
}


TEST_CASE("Write invalid height throws")
{
  auto const width = std::size_t{ 10 };
...
TEST_CASE("Write invalid pixel data throws")
{
  auto const width = std::size_t{ 10 };
  auto const height = std::size_t{ 10 };
  auto const pixel_data = std::vector<std::uint8_t>(width * height * 3 - 1); // Invalid.
  auto oss = std::ostringstream{};
  REQUIRE_THROWS_MATCHES(
    ppm_io::WriteRgbImage(oss, width, height, pixel_data),
    std::invalid_argument,
    utils::ExceptionContentMatcher("pixel data must match width and height"));
}


TEST_CASE("Read invalid filename throws")
{
...
TEST_CASE("Read invalid magic number throws")
{
  ...
  WriteInvalidRgbImage(
    ...
    10,
    10,
    std::vector<std::uint8_t>(10 * 10 * 3));
...
TEST_CASE("Read invalid max value throws")
{
  auto ss = std::stringstream{};
  WriteInvalidRgbImage(
    ss,
  ...
  REQUIRE_THROWS_MATCHES(
    ppm_io::ReadRgbImage(ss, &width, &height, &pixel_data),
    std::runtime_error,
    utils::ExceptionContentMatcher("max value must be 255"));
}


TEST_CASE("Read invalid file size throws")
{
  auto ss = std::stringstream{};
  WriteInvalidRgbImage(
    ss,
...
TEST_CASE("Round-trip")
{
  ...
  auto ss = std::stringstream{};
  ppm_io::WriteRgbImage(ss, write_width, write_height, write_pixels);
  ...
  ppm_io::ReadRgbImage(ss, &read_width, &read_height, &read_pixels);
